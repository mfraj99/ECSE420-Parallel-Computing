...
__global__ void global_queue(int* currLevelNodes_h, int* nodePtrs_h, int* nodeNeighbors_h, int* nodeVisited_h, int* nodeOutput_h, int* nodeGate_h, int* nodeInput_h, int* nextLevelNodes_h, int* numNextLevelNodes_h, int numCurrLevelNodes, int block_queue)
{
	...
	while (index < numCurrLevelNodes) {
		...
		for (int j = nodePtrs_h[node]; j < nodePtrs_h[node + 1]; j++) {
			...
			if (!nodeVisited_h[neighbor]) {
				...
				if (queue_counter < block_queue) {
					queue[queue_counter] = neighbor;
					queue_counter++;
				}
				else {
	...
	for (int l = 0; l < queue_counter; l++) {
		atomicExch(&nextLevelNodes_h[atomicAdd(numNextLevelNodes_h, 1)], queue[queue_counter]);
	}
...
int main() {
	...
	global_queue <<< number_of_blocks, blocksize >>> (cuda_currLevelNodes_h, cuda_nodePtrs_h, cuda_nodeNeighbors_h, cuda_nodeVisited_h, cuda_nodeOutput_h, cuda_nodeGate_h, cuda_nodeInput_h, cuda_nextLevelNodes_h, cuda_numNextLevelNodes_h, numCurrLevelNodes, queue_size);
