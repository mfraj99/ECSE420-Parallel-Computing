...
__global__ void convolution(unsigned char* input, unsigned char* output, int width, int height, int n)
{
    ...
    //////////////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////////////
    //HARD CODED VARIABLE FOR WEIGHT
    float w_cuda[3][3] =
    {
      1,	2,		-1,
      2,	0.25,	-2,
    ...
    if (index<n && index % width != 0 && index % width != width - 1 && row != 1 && index < width * (height - 1)) {
        ...
        // calculate new rgb values for each pixel, a value not necessary, perform the actual convolution
        float r_float = (w_cuda[0][0] * (float)input[png_index_11] + w_cuda[0][1] * (float)input[png_index_12] + w_cuda[0][2] * (float)input[png_index_13] + w_cuda[1][0] * (float)input[png_index_21] + w_cuda[1][1] * (float)input[png_index_22] + w_cuda[1][2] * (float)input[png_index_23] + w_cuda[2][0] * (float)input[png_index_31] + (float)w_cuda[2][1] * (float)input[png_index_32] + w_cuda[2][2] * (float)input[png_index_33]);
        float g_float = (w_cuda[0][0] * (float)input[png_index_11+1] + w_cuda[0][1] * (float)input[png_index_12+1] + w_cuda[0][2] * (float)input[png_index_13+1] + w_cuda[1][0] * (float)input[png_index_21+1] + w_cuda[1][1] * (float)input[png_index_22+1] + w_cuda[1][2] * (float)input[png_index_23+1] + w_cuda[2][0] * (float)input[png_index_31+1] + w_cuda[2][1] * (float)input[png_index_32+1] + w_cuda[2][2] * (float)input[png_index_33+1]);
        float b_float = (w_cuda[0][0] * (float)input[png_index_11+2] + w_cuda[0][1] * (float)input[png_index_12+2] + w_cuda[0][2] * (float)input[png_index_13+2] + w_cuda[1][0] * (float)input[png_index_21+2] + w_cuda[1][1] * (float)input[png_index_22+2] + w_cuda[1][2] * (float)input[png_index_23+2] + w_cuda[2][0] * (float)input[png_index_31+2] + w_cuda[2][1] * (float)input[png_index_32+2] + w_cuda[2][2] * (float)input[png_index_33+2]);
        
        // fix rounding error caused by convolution
        int r = (int)round(r_float);
        int g = (int)round(g_float);
        int b = (int)round(b_float);
        
        // reset rgb values if they fall out of [0,255]
        if (r > 255) {
            r = 255;
        }
        if (r < 0) {
...
int main(int argc, char** argv)
{
    ...
    cudaMalloc((void**)&new_image_cuda, (width-1) * (height-1)* 4 * sizeof(unsigned char));
    ...
    int  number_of_blocks = (width * height / thread_number)+1;
    ...
    convolution<<< grid, block >>> (image_cuda, new_image_cuda, width, height, height * width);
    timer.Stop();
    ...
    // sequential version of the gpu kernel logic for testing
    ...
    // encode image array as a png output and free memory
